...
const AdminExpensesSection: FC = () => {
  ...
  const deleteExpense = useDeleteAdminExpense(businessPlanId);
  ...
  const handleAddLine = () => {
    setRows((prev) => [
      ...prev,
      { id: `new-${Date.now()}`, isNew: true, name: '', taxRate: 14.98, monthlyAmount: 0 },
    ]);
  };
  ...
  const gridRows = [
    ...savedExpenses.map((item) => {
      ...
      const monthly = overrides ?? Array(12).fill(item.monthlyAmount) as number[];
    ...
    ...rows
      ...
      .map((r) => ({
        id: r.id,
        name: r.name || t('fin.adminExp.expensePlaceholder'),
    ...
    <div>
      ...
      <div className="flex flex-wrap items-end gap-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">
        <span className="min-w-[200px]">
          {t('fin.adminExp.expenseName')} <span className="text-red-500">*</span>
        </span>
        <span className="min-w-[90px]">
          {t('fin.adminExp.taxes')} <span className="text-red-500">*</span>
        </span>
      </div>
        ...
        {rows.map((row) => (
          <div key={row.id} className="flex flex-wrap items-center gap-3">
            <Input
              ...
              placeholder={t('fin.adminExp.expensePlaceholder')}
              value={row.name}
      ...
      >
        {t('fin.adminExp.addLine')}
      </Button>

      {/* ── Monthly Forecasts card ── */}
      <div className="bg-card border border-border rounded-2xl p-6">
        <h3 className="text-lg font-heading font-bold text-foreground mb-4">
          {t('fin.adminExp.monthlyForecasts')}
        </h3>
      ...
      <AddToForecastDialog
        ...
        defaultAmount={forecastRow?.monthlyAmount ?? 0}
