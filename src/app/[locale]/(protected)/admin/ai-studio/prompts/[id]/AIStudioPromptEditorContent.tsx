...
export default function AIStudioPromptEditorContent({ locale }: { locale: string }) {
  ...
  const t = {
    ...
    unsaved: language === 'fr' ? 'Non sauvegarde' : 'Unsaved',
    saved: language === 'fr' ? 'Sauvegarde!' : 'Saved!',
    test: language === 'fr' ? 'Tester' : 'Test',
    testing: language === 'fr' ? 'Test en cours...' : 'Testing...',
    history: language === 'fr' ? 'Historique' : 'History',
    improve: language === 'fr' ? 'Ameliorer' : 'Improve',
    analytics: language === 'fr' ? 'Analytics' : 'Analytics',
    systemPrompt: language === 'fr' ? 'PROMPT SYSTEME' : 'SYSTEM PROMPT',
    ...
    temperature: language === 'fr' ? 'Temperature' : 'Temperature',
    provider: language === 'fr' ? 'Fournisseur' : 'Provider',
    model: language === 'fr' ? 'Modele' : 'Model',
    deploy: language === 'fr' ? 'Deploiement' : 'Deployment',
    section: language === 'fr' ? 'Section' : 'Section',
    type: language === 'fr' ? 'Type' : 'Type',
    notFound: language === 'fr' ? 'Prompt non trouve' : 'Prompt not found',
  };
  ...
  const handleImprovementApplied = (improvedSystem: string, improvedUser: string) => {
    ...
    setSuccessMessage(language === 'fr' ? 'Ameliorations appliquees!' : 'Improvements applied!');
    setTimeout(() => setSuccessMessage(null), 3000);
  };
  ...
  const handleRollback = async (versionItem: { id?: string; version: number }) => {
    ...
    try {
      ...
      setSuccessMessage(language === 'fr'
        ? `Retour a la version ${versionItem.version} reussi`
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4">
              ...
              <MonacoPromptEditor
                ...
                placeholder={language === 'fr' ? 'Entrez le prompt systeme...' : 'Enter system prompt...'}
                minHeight={200}
                maxHeight={400}
