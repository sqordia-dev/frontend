...
export default function AIStudioPromptsContent({ locale }: { locale: string }) {
  ...
  const t = {
    ...
    subtitle: language === 'fr'
      ? 'Gerez vos templates de prompts pour la generation de contenu'
    ...
    noResults: language === 'fr' ? 'Aucun prompt trouve' : 'No prompts found',
          ...
          {/* Filters Row */}
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {/* New Prompt Button */}
                <button
                  onClick={() => router.push('/admin/ai-studio/prompts/new')}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4">
            ...
            {prompts.map(prompt => {
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  whileHover={{ y: -2 }}
                  onClick={() => router.push(`/admin/ai-studio/prompts/${prompt.id}`)}
        ...
        {!loading && prompts.length > 0 && (
          <div className="text-center text-sm text-zinc-500 mt-6">
            {language === 'fr' ? `${totalCount} prompts trouves` : `${totalCount} prompts found`}
          </div>
        )}
      </div>
    </div>
