...
const TRANSLATIONS = {
  ...
  fr: {
    ...
    answered: 'repondu',
    sections: 'Sections',
    placeholder: 'Commencez a taper votre reponse...',
    previous: 'Precedent',
    next: 'Suivant',
    questionOf: 'Question {current} sur {total}',
    generatePlan: 'Generer le plan',
    saveError: 'Echec de la sauvegarde. Veuillez reessayer.',
    shortcuts: 'Raccourcis',
    shortcutsTitle: 'Raccourcis clavier',
    shortcutNext: 'Question suivante',
    shortcutPrev: 'Question precedente',
    shortcutCoach: 'Ouvrir le coach IA',
    shortcutSave: 'Sauvegarder la reponse',
...
const STEPS: StepInfo[] = [
  { number: 1, title: 'Identity & Vision', titleFr: 'Identite et Vision', icon: Building2 },
  { number: 2, title: 'The Offering', titleFr: "L'Offre", icon: Lightbulb },
  { number: 3, title: 'Market Analysis', titleFr: 'Analyse de marche', icon: Target },
  { number: 4, title: 'Operations & People', titleFr: 'Operations et Personnel', icon: Users },
  { number: 5, title: 'Financials & Risks', titleFr: 'Finances et Risques', icon: DollarSign },
  { number: 6, title: 'Team', titleFr: 'Equipe', icon: UserPlus },
...
function QuestionnaireInner({ planId, locale }: QuestionnaireInnerProps) {
  ...
  // Fetch questions when persona is available
  useEffect(() => {
    if (planId && persona) {
      fetchQuestions();
    }
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const responseData = (response.data as any)?.value || response.data;
  ...
  const handleAIAction = useCallback(async (questionId: string, action: string) => {
    ...
    try {
      ...
      const transformedText = resData?.transformedText || resData?.value?.transformedText ||
                              resData?.polishedText || resData?.value?.polishedText ||
                              resData?.generatedText || resData?.value?.generatedText;
      if (transformedText) {
  ...
  const handleComplete = useCallback(() => {
    if (planId) {
      router.push(`${basePath}/generation/${planId}`);
    }
  }, [planId, router, basePath]);
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-1 max-w-md mx-4 hidden sm:block">
              <div className="flex items-center justify-between mb-1.5">
                <span className={`text-xs font-medium ${mutedColor}`}>{t.answered}</span>
          ...
          <>
            ...
            >
              ...
              <div className="p-6 space-y-3">
                <ShortcutRow keys={['Ctrl', String.fromCharCode(8594)]} description={t.shortcutNext} theme={theme} />
                <ShortcutRow keys={['Ctrl', String.fromCharCode(8592)]} description={t.shortcutPrev} theme={theme} />
                <ShortcutRow keys={['Ctrl', 'K']} description={t.shortcutCoach} theme={theme} />
                <ShortcutRow keys={['Ctrl', 'S']} description={t.shortcutSave} theme={theme} />
                <ShortcutRow keys={['Ctrl', 'Enter']} description={`${t.shortcutSave} + ${t.shortcutNext}`} theme={theme} />
                <ShortcutRow keys={['Esc']} description={t.shortcutClose} theme={theme} />
                <ShortcutRow keys={['?']} description={t.shortcutHelp} theme={theme} />
              </div>
...
export default function QuestionnaireContent({ locale, planId }: { locale: string; planId: string }) {
  return (
    <QuestionnaireProvider>
      <QuestionnaireInner planId={planId} locale={locale} />
    </QuestionnaireProvider>
  );
}
