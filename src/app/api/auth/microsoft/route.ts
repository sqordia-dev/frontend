...
export async function POST(request: Request) {
  try {
    ...
    // Forward to backend Microsoft OAuth endpoint
    const response = await fetch(`${API_URL}/api/v1/auth/microsoft`, {
      method: 'POST',
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.error || data.message || 'Microsoft authentication failed' },
        { status: response.status }
      );
    }

    // Extract tokens - handle both wrapped and direct response formats
    const jwtToken = data.value?.accessToken || data.value?.token || data.accessToken || data.token;
